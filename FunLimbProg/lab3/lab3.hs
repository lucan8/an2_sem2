...
ackermannf = fix (\f m n -> if m == 0 then n else if n == 0 then f (m - 1) 1 else f (m - 1) (f m n - 1))
...
evenodd (Succ n) = (evenp n, oddp n)

-- evenoddf :: Peano -> (Bool, Bool)
-- evenoddf = fix (\x -> evenodd x)
...
denot (WhileE be stmt) = fix (\w s -> if valueb be s then w (denot stmt s) else s)
...
algs (WhileEF be stmt) = fix (\w s -> if valueb be s then w (stmt s) else s)
